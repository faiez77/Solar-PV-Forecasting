...
if uploaded_file:
    try:
        # Two valid input shapes: a raw single-year Himawari download
        # (2-line metadata header before the real column header), or a
        # combined multi-year file built via pandas concat (clean header,
        # no metadata lines -- e.g. the notebook's himawari_2019_2020
        # output). Detect which one this is rather than assuming.
    ...
    with st.spinner("Fitting forecast model..."):
        m = Prophet(yearly_seasonality=(n_days >= 730), weekly_seasonality=False)  # no real weekly pattern in solar irradiance
        m.fit(df_prophet)
